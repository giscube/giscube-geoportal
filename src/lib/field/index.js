...
function makeFiled ({ layerInfo, fieldInfo, virtual = false } = {}) {
...
export function buildFields (layerInfo) {
  const fields = []
  layerInfo.fields.forEach(fieldInfo => fields.push(makeFiled({ layerInfo, fieldInfo })))
  layerInfo.virtual_fields.forEach(fieldInfo => fields.push(makeFiled({ layerInfo, fieldInfo, virtual: true })))
